...
#start uas
kubectl exec -i $uas -n default -- bash -c "export TERM=xterm && ./sipp -bg -sf uas_mod_orig.xml -rsa $opensips_ip:5060 -i $uas_ip -p 5080" ; 
#kubectl exec -i $uas -n default -- bash -c "echo $?" ;
#start uac
kubectl exec -i $uac -n default -- bash -c "export TERM=xterm && ./sipp -bg  -sf uac_mod.xml $opensips_ip:5060 -trace_screen -s chetan -i $uac_ip -p 5065  -m 100 -r 10 -rp 1000" ;
